...
public class UserMediaTypeVersioningController {
	...
	@GetMapping(value = "/{id}", produces="application/vnd.custom-producer.app.v2+json")
	public UserDtoV2 getUserByIdCustomProducer(@PathVariable("id") @Min(1) Long id) throws UserNotFoundException {
		Optional<User> userById = userService.getUserById(id);
		if(userById.isEmpty()) {
			throw new UserNotFoundException("User not found in model controller V2");
		}
